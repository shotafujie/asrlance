...
- **複数モデル対応**: MLX Whisper、OpenAI Whisper、ReazonSpeech k2、FunASR SenseVoiceSmall、Moonshine (Japanese Base)、Kotoba-Whisper v2.0、rinna/nue-asr、Qwen3-ASR 0.6B、IBM granite-4.0-1b-speech、Gemma 4 E2B (audio) を統一インターフェースで利用可能
...
- **batch_benchmark.py**: 1モデルを1回だけロードして複数音声を一括評価するバッチ版（`kotoba` / `nue` / `qwen` / `granite` / `gemma` 対応）
